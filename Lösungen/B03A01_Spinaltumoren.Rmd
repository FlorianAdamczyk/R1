...
# Blutgruppe und Spinaltumorätiologie: Verschiedene Tabellierungen

Verwendete Software: `r R.version.string`

## Die "Rohdaten"
.... (als Zeichenkettenvektoren), die man *so* nie in R erfassen/einlesen
würde; besseres lernen wir in Kap. 3:
...
Blutgruppe: Levelordnung gemäß alphanumerische Sortierung:
```{r a-BG1}
(BG <- factor(Blutgruppe))
```

Blutgruppe: Levelordnung "selbstbestimmt":
```{r a-BG2}
(BG <- factor(Blutgruppe, levels = c("A", "B", "AB", "0")))
```

Tumorätiologie: Levelordnung gemäß alphanumerische Sortierung
(und aus Platzgründen nur die ersten 20 Einträge gezeigt):
```{r a-TA}
head(TA <- factor(Tumoraetiologie), n = 20)
```
...
## c) Marginalien/Marginalhäufigkeiten (= Randsummen)
### Absolut
Eindimensional / univariat:
```{r c-absolut-univariat}
addmargins(absHK.BG)
addmargins(absHK.TA)
```
